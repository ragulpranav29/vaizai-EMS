...
export class AppService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (rulesCount === 0) {
      await this.ruleModel.insertMany([
        {
          name: 'Critical Issue Assignee',
          triggerEvent: 'high_priority_issue',
          actionType: 'assign_to',
          config: JSON.stringify({ target: 'John Doe' }),
          active: true,
        },
        {
          name: 'Auto QA on Done',
          triggerEvent: 'status_changed',
          actionType: 'assign_to',
          config: JSON.stringify({
            triggerValue: 'done',
            target: 'T-800 Cyberdyne',
          }),
          active: true,
        },
        {
          ...
          config: JSON.stringify({ target: 'Sarah Connor' }),
          active: true,
        },
      ] as any[]);
  ...
  private async seedDatabase() {
    ...
    const projects = await this.projectModel.insertMany([
      {
        name: 'Apollo Lunar Suite',
        key: 'APO',
        description: 'Enterprise space landing guidance control portal',
        lead: 'Sarah Connor',
        status: 'active',
      },
      {
        ...
        key: 'ACM',
        description: 'Next-Gen headless retail platform',
        lead: 'John Doe',
        status: 'active',
      },
    ] as any[]);
    ...
    const sprints = await this.sprintModel.insertMany([
      {
        ...
        endDate: '2026-06-14',
        goal: 'Build telemetry core modules',
      },
      {
        name: 'Apollo Sprint 2 (Active)',
        status: 'active',
        startDate: '2026-06-15',
        endDate: '2026-06-29',
        goal: 'Integrate real-time socket channels',
      },
      {
        name: 'Apollo Sprint 3',
        status: 'future',
        startDate: '2026-06-30',
        endDate: '2026-07-13',
        goal: 'Verify QA compliance & load-testing',
      },
    ] as any[]);
    ...
    const tasks = await this.taskModel.insertMany([
      // Apollo Sprint 1 (Completed Tasks)
      {
        ...
        description:
          'Setup tables for storing altitude, velocity, and fuel logs.',
        status: 'done',
        priority: 'highest',
        assignee: 'John Doe',
        ...
        subTasks: JSON.stringify([
          {
            id: 'st-1',
            title: 'Draft tables relation schema',
            completed: true,
          },
          {
            id: 'st-2',
            title: 'Create DB indices for query acceleration',
            completed: true,
          },
        ]),
        comments: JSON.stringify([
          {
            id: 'c-1',
            author: 'Sarah Connor',
            text: 'Please optimize indices for telemetry queries.',
      ...
      },
      {
        ...
        description:
          'NestJS REST endpoint that handles JSON streams from lunar probe simulator.',
        status: 'done',
        priority: 'high',
        ...
        subTasks: JSON.stringify([
          {
            id: 'st-3',
            title: 'Implement ingestion controller route',
            completed: true,
          },
        ]),
        comments: JSON.stringify([]),
      ...
      // Apollo Sprint 2 (Active Tasks)
      {
        ...
        description:
          'Visual charts using Recharts showing stream status, speed, and fuel burn.',
        status: 'in_progress',
        priority: 'highest',
        assignee: 'Marcus Wright',
        reporter: 'Sarah Connor',
        ...
        subTasks: JSON.stringify([
          {
            id: 'st-4',
            title: 'Setup Recharts responsiveness',
            completed: true,
          },
          {
            id: 'st-5',
            title: 'Implement stream listener feeds',
            completed: false,
          },
        ]),
        comments: JSON.stringify([
          {
            id: 'c-2',
            author: 'John Doe',
            text: 'Use cubic curve settings for fuel speed chart.',
        ...
        activities: JSON.stringify([
          {
            id: 'ac-3',
            action: 'started coding telemetry chart',
      ...
      },
      {
        ...
        description:
          'Hook up React components to receive dashboard events dynamically.',
        status: 'todo',
        priority: 'medium',
        assignee: 'John Doe',
        reporter: 'Sarah Connor',
      ...
      },
      {
        ...
        description:
          'Implement dynamic in-memory PubSub system for single instance environments.',
        status: 'todo',
        priority: 'high',
        assignee: 'Kyle Reese',
        reporter: 'John Doe',
      ...
      // Backlog Tasks
      {
        ...
        description:
          'Test telemetry streaming validation, security filters, and user auth.',
        status: 'backlog',
        priority: 'medium',
    ...
    await this.issueModel.insertMany([
      {
        ...
        description:
          'Connection resets and fails to auto-reconnect due to state cleanup issues.',
        status: 'open',
        priority: 'critical',
        type: 'bug',
        assignee: 'John Doe',
        ...
        comments: JSON.stringify([
          {
            id: 'ic-1',
            author: 'John Doe',
            text: 'Investigating client-side connection states.',
      ...
      },
      {
        ...
        description:
          'Each socket event renders new nodes without cleaning up past canvas instances.',
      ...
      },
      {
        ...
        description:
          'Add capability to post alerts directly to corporate Slack/Teams channels.',
        status: 'open',
        priority: 'medium',
        type: 'feature',
        assignee: 'Kyle Reese',
    ...
    await this.timeLogModel.insertMany([
      {
        ...
        durationHours: 5,
        description: 'Created migration scripts and tested locally',
        date: '2026-06-05',
        memberName: 'John Doe',
        billable: true,
      },
      {
        ...
        taskTitle: tasks[0].title,
        durationHours: 5,
        description: 'Refined indices and schema docs',
        date: '2026-06-06',
        memberName: 'John Doe',
        billable: true,
      },
      {
        ...
        durationHours: 8,
        description: 'Drafted Canvas components and test feed',
        date: '2026-06-20',
        memberName: 'Marcus Wright',
        billable: false,
      },
    ] as any[]);
    ...
    await this.allocationModel.insertMany([
      {
        memberName: 'John Doe',
        projectName: 'Apollo Lunar Suite',
        allocatedHours: 30,
        startDate: '2026-06-15',
        endDate: '2026-06-29',
      },
    ...
    await this.messageModel.insertMany([
      {
        channel: 'general',
        text: 'Welcome to the Enterprise Project Management chat!',
        sender: 'System',
        timestamp: new Date(),
      },
      {
        channel: 'general',
        text: 'Great progress on Apollo Sprint 1 team!',
        sender: 'Sarah Connor',
        timestamp: new Date(Date.now() - 1000 * 60 * 60),
      },
      {
        channel: 'general',
        text: 'Thanks Sarah. Working on WebSocket integrations now.',
        sender: 'John Doe',
  ...
  private serializeTaskJson(data: Partial<Task>): any {
    ...
    if (data.labels && Array.isArray(data.labels))
      result.labels = JSON.stringify(data.labels);
    if (data.subTasks && Array.isArray(data.subTasks))
      result.subTasks = JSON.stringify(data.subTasks);
    if (data.comments && Array.isArray(data.comments))
      result.comments = JSON.stringify(data.comments);
    if (data.attachments && Array.isArray(data.attachments))
      result.attachments = JSON.stringify(data.attachments);
    if (data.activities && Array.isArray(data.activities))
      result.activities = JSON.stringify(data.activities);
    return result;
  }
  ...
  async updateTask(id: string, data: Partial<Task>) {
    ...
    await this.taskModel
      .findByIdAndUpdate(id, serialized, { new: true })
      .exec();
  ...
  // --- Time Logs ---
  async getTimeLogs() {
    return this.timeLogModel.find().exec();
  }
  async logTime(data: Partial<TimeLog>) {
    ...
    if (log.taskId) {
      ...
      if (task) {
        const totalSpent =
          Number(task.timeSpent || 0) + Number(log.durationHours || 0);
        await this.taskModel
          .findByIdAndUpdate(
            log.taskId,
      ...
      try {
        ...
        if (event === 'status_changed') {
          ...
          if (rule.actionType === 'assign_to') {
            ...
            console.log(
              `[Automation Triggered] Rule "${rule.name}" updated task ${entity.key} assignee to: ${config.target}`,
            );
          }
        }
        ...
        if (event === 'high_priority_issue') {
          if (rule.actionType === 'assign_to') {
            ...
            console.log(
              `[Automation Triggered] Rule "${rule.name}" updated issue ${entity.key} assignee to: ${config.target}`,
            );
          }
        }
      } catch (err) {
  ...
  async getReportsMetrics() {
    ...
    if (activeSprint) {
      ...
      if (sprintTasks.length > 0) {
        ...
        sprintCompletionRate = Math.round(
          (doneSprintTasks.length / sprintTasks.length) * 100,
        );
      }
    }
    ...
    const resolutionReport = members.map((m) => {
      const resolvedCount = issues.filter(
        (i) => i.assignee === m.name && i.status === 'resolved',
      ).length;
      const closedCount = issues.filter(
        (i) => i.assignee === m.name && i.status === 'closed',
      ).length;
      return {
        name: m.name,
  ...
  async getDashboardMetrics() {
    ...
    if (activeSprint) {
      ...
      if (activeSprintTasks.length > 0) {
        const completedActiveTasks = activeSprintTasks.filter(
          (t) => t.status === 'done',
        );
        sprintProgress = Math.round(
          (completedActiveTasks.length / activeSprintTasks.length) * 100,
        );
      }
    }
    ...
    const recentActivities = [
      ...
      {
        id: '3',
        user: 'T-800 Cyberdyne',
        action: 'reported critical bug',
        target: 'BUG-101',
        details: 'Socket failures',
        time: '3h ago',
      },
      {
        id: '4',
        ...
        target: 'APO-1',
        details: 'Telemetry schema design',
        time: 'Yesterday',
      },
    ];

    const todaysWork = tasks.filter(
      (t) => t.assignee === 'John Doe' && t.status === 'in_progress',
    );

    const upcomingDeadlines = [
      {
        id: 'dl-1',
        title: 'Sprint 2 Telemetry Chart Deliverable',
        date: activeSprint?.endDate || '2026-06-29',
        severity: 'high',
      },
      {
        id: 'dl-2',
        title: 'QA Code Freeze - Apollo Ingestion',
        date: '2026-07-02',
        severity: 'medium',
      },
    ];
